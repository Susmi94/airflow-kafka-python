...
#             """, (uuid(), row.first_name, row.last_name, row.gender, row.address,
#                   row.post_code, row.email, row.username, row.registered_date, row.phone, row.picture))
#
#             print(f"Data inserted for {row.first_name} {row.last_name}")
#         except Exception as e:
#             logging.error(f'Could not insert data for {row.first_name} {row.last_name} due to {e}')
#
#
...
#             .option("assign", """{"users_created":[0,1,2]}""") \
#             .load()
...
#
#
# def process_batch(data, batch_id):
#     session = create_cassandra_connection()
#     if session is not None:
#         insert_data(session, data)
#     else:
#         logging.error("Failed to connect to Cassandra within foreachBatch")
#
#
